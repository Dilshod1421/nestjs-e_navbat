...
import { AddMinutesToDate, dates, decode, encode } from '../helpers/crypto';
...
export class SpecialistService {
  ...
  async auth(authSpecialistDto: AuthSpecialistDto) {
    ...
    if (specialist) {
      ...
      if (oldOtp) {
        const updateOtp = await this.otpService.update(
          oldOtp._id.toString(), {
          otp,
          expiretion_time: expiretion_time.toString(),
          verified: false
        });
      ...
      if (!result.verified) {
        if (dates.compare(result.expiretion_time, currentDate)) {
          if (otp === result.otp) {
            ...
            await this.otpService.update(obj.otp_id, { verified: true })
            ...
            if (existstoken) {
              await this.tokenService.update(existstoken._id.toString(), { hashed_refresh_token })
            }
            else {
